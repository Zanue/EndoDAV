...
class DPTHead(nn.Module):
    def __init__(self, in_channels, features=128, use_bn=False, out_channels=[96, 192, 384, 768], use_clstoken=False, inv_sigmoid=False, disable_conv_head=False):
    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        outputs = {}
        if not self.disable_conv_head:
            sig_sign = -1 if self.inv_sigmoid else 1
            outputs[("disp", 3)] = self.sigmoid(sig_sign*self.conv_depth_4(path_4))
            outputs[("disp", 2)] = self.sigmoid(sig_sign*self.conv_depth_3(path_3))
            outputs[("disp", 1)] = self.sigmoid(sig_sign*self.conv_depth_2(path_2))
            outputs[("disp", 0)] = self.sigmoid(sig_sign*self.conv_depth_1(path_1))
        else:
            ...
            outputs[("disp", 1)] = F.interpolate(outputs[("disp", 0)], scale_factor=0.5, mode="bilinear", align_corners=True)
            outputs[("disp", 2)] = F.interpolate(outputs[("disp", 1)], scale_factor=0.5, mode="bilinear", align_corners=True)
            outputs[("disp", 3)] = F.interpolate(outputs[("disp", 2)], scale_factor=0.5, mode="bilinear", align_corners=True)
...
class endodac(nn.Module):
    ...
    def __init__(self, 
        ...
        self.pretrained = self.backbone[self.backbone_size]
        # self.pretrained = DINOv2(model_name=self.model_name[self.backbone_size])
        ...
        self.depth_head = DPTHead(self.embedding_dim, self.depth_head_feature, use_bn, \
                                  out_channels=self.depth_head_out_channel, \
                                    use_clstoken=use_cls_token, \
                                        ...
                                        disable_conv_head=disable_conv_head)
    ...
    def forward(self, pixel_values):
        ...
        x_norm = self.normalize(pixel_values)
        
        features = self.pretrained.get_intermediate_layers(x_norm, 4, return_class_token=True)
    ...
    @torch.no_grad()
    def infer_video_depth(self, frames, batch_size=8, device='cuda'):
        self.eval()
        frame_height, frame_width = frames[0].shape[:2]
        depth_list = []
        for chunk in range(0, len(frames), batch_size):
            frames_chunk = frames[chunk:chunk+batch_size].astype(np.float32) / 255.
            frames_chunk = np.transpose(frames_chunk, (0, 3, 1, 2))
            frames_chunk = torch.from_numpy(frames_chunk).float().to(device)
            depth = self.forward(frames_chunk)[("disp", 0)] # [B, 1, H, W]
            depth = F.interpolate(depth, size=(frame_height, frame_width), mode='bilinear', align_corners=True)[:, 0] # [B, H, W]
