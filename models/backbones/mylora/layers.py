...
class Linear_SSB(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self, 
        in_features: int, 
        ...
        **kwargs
        ...
        # Actual trainable parameters
        if r > 0:
            self.lora_A = nn.Parameter(torch.ones(in_features, 1))
    ...
    def forward(self, x: torch.Tensor):
        def T(w):
            return w.T if self.fan_in_fan_out else w
        if self.r > 0 and not self.merged:
            result = F.linear(x, T(self.lora_A.view(1,-1) * self.weight * self.lora_B), bias=self.bias)
            return result
        else:
            return F.linear(x, T(self.weight), bias=self.bias)


class Linear_FLora(nn.Linear, LoRALayer):
...
class DashLinear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self, 
        in_features: int, 
