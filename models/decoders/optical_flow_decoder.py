...
# use nn.ModuleDict() because of dp
class PositionDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales = range(4) , num_output_channels=2, use_skips=True):
        super(PositionDecoder, self).__init__()
        ...
        for s in self.scales:
            self.convs[f"position_conv_{s}"] = self.conv (self.num_ch_dec[s], self.num_output_channels, kernel_size = 3, padding = 1)
            # init flow layer with small weights and bias
            self.convs[f"position_conv_{s}"].weight = nn.Parameter(Normal(0, 1e-5).sample(self.convs[f"position_conv_{s}"].weight.shape))
            self.convs[f"position_conv_{s}"].bias = nn.Parameter(torch.zeros(self.convs[f"position_conv_{s}"].bias.shape))
    ...
    def forward(self, input_features):
        ...
        for i in range(4, -1, -1):
            ...
            if i in self.scales:
                self.outputs[("position", i)] = self.convs[f"position_conv_{i}"](x)
