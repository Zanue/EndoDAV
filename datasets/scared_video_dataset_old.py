...
class SCAREDVideoDataset(VideoDataset):
    def __init__(self, *args, **kwargs):
        super(SCAREDVideoDataset, self).__init__(*args, **kwargs)
...
class SCAREDRAWVideoDataset(SCAREDVideoDataset):
    def __init__(self, *args, **kwargs):
        self.idx_shift = 1
        super(SCAREDRAWVideoDataset, self).__init__(*args, **kwargs)

    def get_image_path(self, folder, frame_index, side):
        # f_str = "frame_data{:06d}{}".format(frame_index-self.idx_shift, self.img_ext)
        f_str = "{:010d}{}".format(frame_index-self.idx_shift, self.img_ext)
        sequence = folder[7]
    ...
    def get_depth(self, folder, frame_index, side, do_flip):
        f_str = "scene_points{:06d}.tiff".format(frame_index-self.idx_shift)
        sequence = folder[7]
        data_splt = "train" if int(sequence) < 8 else "test"
    ...
    def get_pose(self, folder, frame_index):
        f_str = "frame_data{:06d}.json".format(frame_index-self.idx_shift)
        sequence = folder[7]
        data_splt = "train" if int(sequence) < 8 else "test"
