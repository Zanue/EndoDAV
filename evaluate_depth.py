...
def evaluate(opt):
    ...
    if opt.visualize_depth:
        vis_dir = os.path.join(opt.load_weights_folder, 'eval', opt.eval_split, "vis_depth")
        os.makedirs(vis_dir, exist_ok=True)
        depth_dir = os.path.join(opt.load_weights_folder, 'eval', opt.eval_split, "depth")
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader)):
            ...
            if opt.visualize_depth:
                ...
                seq_dir = os.path.join(vis_dir, sequence)
                os.makedirs(seq_dir, exist_ok=True)
                seq_depth_dir = os.path.join(depth_dir, sequence)
                os.makedirs(seq_depth_dir, exist_ok=True)
                vis_file_name = os.path.join(seq_dir, keyframe + frame_id + ".png")
                cv2.imwrite(vis_file_name, vis_pred_depth)
                # depth_file_name = os.path.join(seq_depth_dir, keyframe + frame_id + ".png")
                # cv2.imwrite(depth_file_name, (1000.*pred_depth).astype(np.uint16))
                depth_file_name = os.path.join(seq_depth_dir, keyframe + frame_id + ".npy")
                np.save(depth_file_name, pred_depth)
